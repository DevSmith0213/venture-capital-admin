...
const BannerTable = ({ data, setLoading }: IProps) => {
    ...
    confirm({
      ...
      async onOk() {
        try {
          ...
          openNotificationWithIcon('success', "Banner", `It has been successfully ${type}d.`)
          setLoading(false)
          router.refresh();
          window.location.reload();
        } catch (error) {
          console.log(error);
          openNotificationWithIcon('error', "Banner", `It has been ${type} failed`)
          setLoading(false)
        }
      },
      onCancel() {
  ...
  const handleCreateItem = async (type: string, id: any) => {
    ...
    try {
      if (imageFile) {
        var logo: any = await uploadFileHandler("banner", imageFile);
      ...
      const response: any = await fetch('/api/bannerCreateEdit', {
        method: 'post',
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        setLoading(false)
        openNotificationWithIcon('error', "Banner", `Create failed`)
        throw new Error(`response status: ${response.status}`);
      }
      
      await response.json();
      setTimeout(() => {
        openNotificationWithIcon('success', "Banner", `It has been successfully created.`)
        setImageFile(null);
        setLoading(false)
        setImage("")
        setLink("")
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        <div className="w-full flex justify-end">
          <Button onClick={() => setOpen(true)} className="flex items-center bg-[#1c2434] text-white">
          ...
          <Modal
            ...
            footer={(_: any, { OkBtn, CancelBtn }: any) => (
              <>
                <Button className="bg-[#1c2434] text-white" onClick={() => handleCreateItem("create", null)} disabled={link == ""}>Create Banner</Button>
              </>
            )}
          >
            <Form.Item
              name="link"
              hasFeedback
              label={<span className="text-black dark:text-white">Banner Link</span>}
              rules={[{ required: true }, { type: 'string' }]}
            >
              <Input value={link} onChange={(event: any) => setLink(event.target.value)} placeholder="Please input Title" />
            </Form.Item>
            <Form.Item
              name="Image"
              hasFeedback
              ...
              <tr key={item._id}>
                ...
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    <Button onClick={() => { setSelectedId(item._id); setImage(item.image); setLink(item.link) }} className="flex items-center bg-[#1c2434] text-white"><EditOutlined /> Edit</Button>
                    <Modal
                      title="Updated Banner"
                      centered
                      open={selectedId == item._id}
                      onCancel={() => { setSelectedId(null); setImage(""); setLink("") }}
                      footer={(_: any, { OkBtn, CancelBtn }: any) => (
                        <>
                          <Button onClick={() => { setSelectedId(null); setImage(""); setLink("") }}>Cancel</Button>
                          <Button className="bg-[#1c2434] text-white" onClick={() => handleCreateItem("change", item._id)}>Change Banner</Button>
                        </>
                      )}
                    >
                      <Form.Item
                        ...
                        rules={[{ required: true }, { type: 'string' }]}
                      >
                        <Input value={link} defaultValue={item.link} onChange={(event: any) => setLink(event.target.value)} placeholder="Please input Title" />
                      </Form.Item>
                      <Form.Item
                        name="Image"
                        hasFeedback
