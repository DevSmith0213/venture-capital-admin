...
const BlogTable = ({ data, perPage, currentPage, setCurrentPage }: IProps) => {
  ...
  const handleItem = async (id: any, type: string) => {
    confirm({
      ...
      async onOk() {
        try {
          const response = await axios.post("/api/changeBlogFilter", { route: "blog", id, type });
          router.refresh();
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        <div className="w-full flex justify-end">
          <Link className="mb-5.5 inline-block bg-[#1c2434] text-white px-4 py-2 rounded-lg" href="/blog/create">
            Create New Blog
          </Link>
        </div>
              ...
              <tr key={item._id}>
                ...
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    ...
                    <Link className="hover:text-primary" href={`/blog/edit_${item._id}`}>
                      <EditOutlined />
                    </Link>
